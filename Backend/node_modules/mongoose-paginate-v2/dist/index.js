...
function isSessionInTransaction(session) {
  ...
  var driverSession = session.session;
...
function hasActiveSession(model, queryOptions) {
  ...
  var base = model && model.db && model.db.base;
  var als = base && base.transactionAsyncLocalStorage;
...
function paginate(query, options, callback) {
  ...
  function buildDocsPromise() {
    ...
    var mQuery = model[customFind](query, projection, queryOptions);
    if (populate) {
    ...
    var execPromise = mQuery.exec();
    if (lean && leanWithId) {
      execPromise = execPromise.then(function (docs) {
  ...
  var hasSession = hasActiveSession(model, queryOptions);
  var resultPromise;
  if (hasSession) {
    resultPromise = Promise.resolve(countPromise).then(function (count) {
      return buildDocsPromise().then(function (docs) {
        return buildResult(count, docs);
      });
    });
  } else {
    docsPromise = buildDocsPromise();
    resultPromise = Promise.all([countPromise, docsPromise]).then(function (values) {
